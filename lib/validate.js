...
/** Like int, but keeps the fraction — the portrait zoom moves in steps of 0.05. */
...
export function sanitizeConfig(input) {
  ...
  // An absent video block means "use the defaults", the same convention frames
  // follow. An explicit empty list still means the owner cleared it.
  const videoIn = input.video === undefined ? DEFAULT_CONFIG.video : (input.video || {});
  const videoUrl = mediaUrl(videoIn.url);
  const sourcesIn = Array.isArray(videoIn.sources) ? videoIn.sources : [];
  if (sourcesIn.length > 12) throw new BadInput('At most 12 background clips.');
  const sources = sourcesIn.map(mediaUrl).filter(Boolean);

  return {
    title: text(input.title, DEFAULT_CONFIG.title),
    heading: text(input.heading, DEFAULT_CONFIG.heading),
    ...
    video: {
      url: videoUrl,
      sources,
      enabled: bool(videoIn.enabled, false) && Boolean(videoUrl || sources.length)
    },
    motion: {
      mode,
    ...
    chrome: {
      ...
      portraitZoom: num(input.chrome?.portraitZoom, DEFAULT_CONFIG.chrome.portraitZoom, 1, 4)
    },
